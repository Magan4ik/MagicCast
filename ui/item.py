...
class Item(Entity):
    def __init__(self, name: str, img: pyglet.image.AbstractImage, batch: Optional[pyglet.graphics.Batch], stackable: bool = True):
    ...
    def __copy__(self):
        item = Item(self.name, self.image, self.batch, self.stackable)
        item.x = self.x
        item.y = self.y
...
class SpellItem(Item):
    def __init__(self, icon: pyglet.image.AbstractImage, spell: BaseSpell, batch: Optional[pyglet.graphics.Batch]):
        super().__init__(spell.name, icon, batch=batch, stackable=False)
        self.spell = spell
